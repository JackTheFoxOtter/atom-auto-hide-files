...
export default {
    ...
    add_treeview_observer() {
        if (!this.observer_initialized) {
            var treeViewNode = document.querySelector(".tree-view")
            if (treeViewNode !== null) {
                this.mutationObserver.observe(treeViewNode, {childList: true, subtree: true});
                this.observer_initialized = true;
            } else {
                atom.notifications.addWarning(" Auto-Hide couldn't start: Treeview not open. Open treeview and toggle Auto-Hide to try again.");
                this.active = false;
            }
        }
    },
